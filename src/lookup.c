...
int subclan_lookup(const char *name)
{
    int subclan;
    
    for( subclan = 0; subclan < MAX_SUBC; subclan++ )
    {
    	if( LOWER(name[0]) == LOWER(subclan_table[subclan].name[0])
    	&&  !str_prefix(name, subclan_table[subclan].name) )
    		return subclan;
    }
