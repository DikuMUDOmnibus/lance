...
const struct olc_help_type help_table[] =
{
    ...
    {	"class",	class_flags,	 "Class-use restrictions."	 },
    {	"spec",		spec_table,	 "Available special programs." 	 },
    {	"sex",		sex_flags,	 "Sexes."			 },
    {	"act",		act_flags,	 "Mobile attributes."		 },
    {	"act2",		act2_flags,	 "Mobile attributes2."		 },
...
bool do_rlist(CHAR_DATA *ch, char *argument)
{
...
bool do_mlist(CHAR_DATA *ch, char *argument)
{
...
bool do_olist(CHAR_DATA *ch, char *argument)
{
...
REDIT( redit_mshow )
{
    ...
    {
       send_to_char( "REdit: Needs to be a number.\n\r", ch);
       return FALSE;
    }
...
AEDIT( aedit_range )
{
    ...
    if(iupper < ilower)
    {
    	send_to_char("The upper range must be above the lower range.\n\r", ch);
    	return FALSE;
    }
    
    pArea->low_range = ilower;
    pArea->high_range = iupper;
    send_to_char("AEdit: Range set.\n\r", ch);
    return TRUE;
}
...
REDIT( redit_show )
{
    ...
    sprintf( buf, "Description 1:\n\r%s", pRoom->description1 );
    strcat( buf1, buf );

    sprintf( buf, "Description 2:\n\r%s", pRoom->description2 );
    strcat( buf1, buf );
...
bool change_exit( CHAR_DATA *ch, char *argument, int door )
{
	...
	{
	    send_to_char( "REdit:  Item isn't a key.\n\r", ch );
	    return FALSE;
	}
...
REDIT( redit_desc1 )
{
    ...
    if ( argument[0] == '\0' )
    {
	string_append( ch, &pRoom->description1 );
	return TRUE;
    }

    send_to_char( "Syntax:  desc1\n\r", ch );
    return FALSE;
}
...
REDIT( redit_format )
{
    ...
    if( pRoom->description1[0] != '\0' )
    {
	pRoom->description1 = format_string( pRoom->description1 );
    }
    if( pRoom->description2[0] != '\0' )
    {    
...
const struct wear_type wear_table[] =
{
    ...
    {	WEAR_RIGHT,	ITEM_WIELD		},
    {	WEAR_LEFT,	ITEM_HOLD		},
    {	WEAR_ANKLE,	ITEM_WEAR_ANKLE		},
...
REDIT( redit_oreset )
{
	...
	pReset->command	= 'P';
	pReset->arg1	= pObjIndex->vnum;
	pReset->arg2 = 1; /* Changed from 0 so objects actually load */
	...
	/*
	 * Disallow loading a sword(WEAR_RIGHT) into WEAR_HEAD.
	 */
...
OEDIT( oedit_autoset )
{
    ...
    if(pObj->level < 1)
    {
        send_to_char("You must set the object's level.\n\r", ch);
        return FALSE;
    }
    ...
    if(pObj->item_type == ITEM_WEAPON)
    {
        if(pObj->level <= 50)
        {
            avg = interpolate(pObj->level, 4, 25);
            dice = 1;
            size = 4;
        }
        else
        {
            avg = interpolate(pObj->level, 6, 25);
            dice = 5;
            size = 10;
        }
        
        for(; (size + 2) * dice / 2 < avg; dice++)
        {
            if(dice + 1 == size)
            {
                dice = 1;
        ...
        send_to_char("Values set for weapon.\n\rReview the changes before accepting.\n\r", ch);
        return TRUE;
    } /* End of autoset for weapons */
    
    if(pObj->item_type == ITEM_ARMOR)
    {
        /* AC should be roughly 30% of the item's level */
        size = pObj->level * (pObj->level > 50 ? .25 : .3);

        pObj->value[0] = pObj->value[1] = pObj->value[2] = size;

        /* Magic AC should be significantly lower */
        if(size >= 10)
            pObj->value[3] = UMAX(size / 2, size - 10);
        else
            pObj->value[3] = UMAX(0, size - 5);
        
        send_to_char("Values set for armor.\n\rReview the changes before accepting.\n\r", ch);
        return TRUE;
    } /* End of autoset armor */
    send_to_char("You must set the object type to weapon or armor.\n\r", ch);
    return FALSE;
}
...
OEDIT( oedit_class )      /* Moved out of oedit() due to naming conflicts -- Hugin */
{
    OBJ_INDEX_DATA *pObj;
    int value;
    int i;
	...
	{
	    for( i = 0 ; class_flags[i].name != NULL ; i++ )
	    {
	    	value = flag_value( class_flags, class_flags[i].name );
...
MEDIT( medit_autoset )
{
    ...
    if( pMob->level < 1 )
    {
    	send_to_char( "You must set the level of the mob first.\n\r", ch );
    	return FALSE;
    }
    ...
    value = -(pMob->level * 7 * (pMob->level <= 50 ? .4 : .3));
    value += 50;
    pMob->ac[AC_PIERCE] = pMob->ac[AC_BASH] = pMob->ac[AC_SLASH] = value;
    if(value <= -30)
        pMob->ac[AC_EXOTIC] = UMIN(value / 2, value + 10);
    else
        pMob->ac[AC_EXOTIC] = UMAX(value / 1.5, value + 5);

/*    
    pMob->ac[AC_PIERCE] = ( 95 - 6*(level-1) );
    pMob->ac[AC_BASH]	= ( 95 - 6*(level-1) );
    pMob->ac[AC_SLASH]	= ( 95 - 6*(level-1) );
    pMob->ac[AC_EXOTIC]	= ( 95 - 3*(level-1) );
*/    
    ...
    strcpy( buf, "head arms legs heart brains guts hands feet fingers ear eye" );
     
    value = flag_value( part_flags, buf );
    pMob->parts |= value;
    ...
    if( pMob->level <= 30 )
    {
    	pMob->hit[DICE_NUMBER]	= (pMob->level / 3 );
    	pMob->hit[DICE_TYPE]	= (pMob->level * 2 );
    } 
    else
    {
    	pMob->hit[DICE_NUMBER]	= 10;
    	pMob->hit[DICE_TYPE] = (pMob->level * 3 );
    }
    
    if( pMob->level <= 15 )
    	pMob->hit[DICE_BONUS]	= (pMob->level * 10 );
    else
    	pMob->hit[DICE_BONUS]	= (pMob->level * ((pMob->level/2) - ((pMob->level-30)/4)));
    ...
    if(pMob->level <= 50)
    {
        value = interpolate(pMob->level, 4, 30);
        dice = 1;
        size = 4;
    }
    else
    {
        value = interpolate(pMob->level, 6, 27);
        dice = 5;
        size = 10;
    }
        
    for(; (size + 2) * dice / 2 < value; dice++)
    {
        if(dice + 1 == size)
        {
            dice = 1;
    ...
    pMob->damage[DICE_NUMBER] = dice;
    pMob->damage[DICE_TYPE] = size;
    pMob->damage[DICE_BONUS] = pMob->level * .6;

/*
    if( level < 5 )
    	pMob->damage[DICE_NUMBER] = 1;
    else if( level < 14 )
    	pMob->damage[DICE_NUMBER] = 2;
    else if( level < 20 )
    	pMob->damage[DICE_NUMBER] = 3;
    else if( level < 30 )
    	pMob->damage[DICE_NUMBER] = 4;
    else if( level <= 50 )
    	pMob->damage[DICE_NUMBER] = 5;
    else if( level <= 90 )
    	pMob->damage[DICE_NUMBER] = 6;
    else
    	pMob->damage[DICE_NUMBER] = 7;
    	
    if( (level == 1) || (level == 2) )
    	pMob->damage[DICE_TYPE] = 2;
    else if( level <= 5 )
    	pMob->damage[DICE_TYPE] = 3;
    else if( level > 5 )
    	pMob->damage[DICE_TYPE] = (level/pMob->damage[DICE_NUMBER]);
    	
    pMob->damage[DICE_BONUS] = (level/2);
*/     
    
    send_to_char( "Automatically set, Hitroll, Parts, Damdice, Manadice, and AC.\n\r", ch );
