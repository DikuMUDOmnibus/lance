...
void do_aedit( CHAR_DATA *ch, char *argument )
{
	...
	{
		send_to_char( "AEdit : Insufficient security.\n\r", ch );
		return;
	}
...
void do_alist( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf, "{r[{c%3s{r] [{y%-27s{r] ({g%-5s{w-{m%5s{r) [{W%-10s{r] {b%3s {r[{D%-10s{r]{x\n\r",
       "Num", "Area Name", "lvnum", "uvnum", "Filename", "Sec", "Builders" );
    add_buf(buff, buff);
    
    for ( pArea = area_first; pArea; pArea = pArea->next )
    {
	sprintf( buf, "{r[{c%3d{r] {y%-29.29s {r({g%-5d{w-{m%5d{r) {W%-12.12s {r[{b%d{r] [{D%-10.10s{r]{x\n\r",
	     pArea->vnum,
	     pArea->name,
	     pArea->min_vnum,
	     pArea->max_vnum,
...
*/
    ...
    page_to_char( buf_string(buff), ch );
    return;
}
