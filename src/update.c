...
void advance_level( CHAR_DATA *ch, bool hide )
{
    ...
    if(get_curr_stat(ch, STAT_INT) >= 21)
        add_mana += number_range(5, get_curr_stat(ch, STAT_INT) / 2);
...
void gain_exp( CHAR_DATA *ch, int gain )
{
	...
	if(ch->level == 15)
	{
	    ...
	    || !str_cmp(class_table[ch->class].name, "templar"))
	    {
	    /* Send to heathen */
	    	ch->clan = clan_lookup("heathen");
	    	send_to_char(
"You have been marked as a heathen. You should seek a member of the {GHoly Order{x\n\r\
to discuss joining the clan.", ch);
	    }
	    else if(!str_cmp(class_table[ch->class].name, "mage")
	    || !str_cmp(class_table[ch->class].name, "bmage"))
	    /* Send to renegade */
	    {
	    	ch->clan = clan_lookup("renegade");
	    	send_to_char(" You are now a well known mage, and called a renegade! Consider contacting\n\r\
members of the {WConclave{x or there will be some consequences.", ch);
	    }
	    else
	    {
	    	ch->clan = clan_lookup("loner");
	    	send_to_char("You are a {Yloner{x. You are now eligible to petition for other clans.\n\r", ch);
	    }
	}
	save_char_obj(ch);
    }
...
void gain_condition( CHAR_DATA *ch, int iCond, int value )
{
	...
	case COND_HUNGER:
	    condition = ch->pcdata->condition[iCond];
	    if(condition < -8)
	    	send_to_char("You are {RDYING{x of hunger!\n\r", ch);
	    else if(condition < -4)
	    	send_to_char("Your stomach warns of a Cataclysm.\n\r", ch);
	    else
	...
	case COND_THIRST:
	    condition = ch->pcdata->condition[iCond];
	    if(condition < -8)
	    	send_to_char("You are {REXTREMELY{x thirsty!\n\r", ch);
	    else if(condition < -4)
	    	send_to_char("Your throat feels like the Plains of Dust.\n\r", ch);
	    else
...
{   
	...
	if ( ch->position >= POS_STUNNED )
	{
	    ...
	    if(!IS_NPC(ch) && (ch->pcdata->condition[COND_HUNGER] <= -8
	      || ch->pcdata->condition[COND_THIRST] <= -8))
	    {
	    	ch->hit -= UMAX(ch->level/2, 8);
	    	ch->mana -= UMAX(ch->level/2, 8);
	    	if(ch->hit < 1)
	...
	if( ch->pet != NULL && 
	   (ch->position == POS_RIDING || IS_SET( ch->act, PLR_MOUNTED )) &&
...
/*	    if (IS_IMMORTAL(ch))
		ch->timer = 0;
*/
	    if ( ++ch->timer >= 10 && !IS_IMMORTAL(ch) && !IS_SET( ch->comm, COMM_AFK ) )
	    {
	    ...
	    gain_condition( ch, COND_HUNGER, ch->size > SIZE_MEDIUM ? -1 : -1);
	}
	...
	for ( paf = ch->affected; paf != NULL; paf = paf_next )
	{
		...
		||   paf_next->duration > 0 )
			...
			if ( paf->bitvector == AFF_HALLUCINATING && paf->where == TO_AFFECTS2)
			{
			    char_from_room( ch );
			    char_to_room( ch, ch->trip_room );
    ...
    for ( ch = char_list; ch != NULL; ch = ch_next )
    {
        if(!IS_VALID(ch))
        {
            bug("char_update: Trying to use invalidated char.", 0);
            break;
        }
        ch_next = ch->next;
	...
	if( ch->violent > 1 )
	    ch->violent = ch->violent - 1;
	
	if( ch->violent == 1 )
	{
	    REMOVE_BIT( ch->act, PLR_VIOLENT );
	    ch->violent = 0;
	...
	if( ch->slain == 1 )
	{
	    ...
	    ch->slain = 0;
	    send_to_char( "You are fair game for PK again.\n\r", ch );
	}
    }
...
{   
	...
	case ITEM_CONTAINER: 
		...
		"$p flickers and vanishes, spilling its contents on the floor.";
	    	...
	    	message = "$p crumbles into dust, spilling its contents everywhere.";
	    else
	...
	if ( obj->carried_by != NULL )
	{
	    ...
	    {
	    	act( message, obj->carried_by, obj, NULL, TO_ALL );
