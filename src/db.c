...
sh_int			gsn_human;
sh_int			gsn_silvanesti;
sh_int			gsn_qualinesti;
sh_int			gsn_kagonesti;
sh_int			gsn_dimernesti;
sh_int			gsn_dargonesti;
sh_int			gsn_hylar;
sh_int			gsn_neidar;
sh_int			gsn_aghar;
sh_int			gsn_gnome;
...
sh_int			gsn_right_hand;
sh_int			gsn_left_hand;
 ...
 */
#define			MAX_STRING	2999920
#define			MAX_PERM_BLOCK	191072
       ...
       switch ( UPPER(word[0]) )
       {
           ...
           case 'R':
            if(!str_cmp(word, "Range"))
            {
            	...
            	if(pArea->high_range < pArea->low_range)
            	    pArea->high_range = pArea->low_range;
            }
            break;
           case 'S':
...
void load_resets( FILE *fp )
{
	...
	switch( pReset->command )
	{
		...
		switch ( pReset->arg3 )
		{
			...
			case 3: SET_BIT( pexit->rs_flags, EX_CLOSED | EX_HIDDEN );
				SET_BIT( pexit->exit_info, EX_CLOSED | EX_HIDDEN );break;
			case 4: SET_BIT( pexit->rs_flags, EX_CLOSED | EX_LOCKED | EX_HIDDEN );
				SET_BIT( pexit->exit_info, EX_CLOSED | EX_LOCKED | EX_HIDDEN ); break;
		}
		break;
...
void load_rooms( FILE *fp )
{
	...
	pRoomIndex->description1	= fread_string( fp );
...
/* */
		...
		if ( pObjIndex->count < limit || number_range(0,4) == 0 )
		{
		    ...
		    &&   pReset->command == 'E' 
		    &&   pObj->level < LastMob->level - 30 && pObj->level < 35))
			fprintf(stderr,
...
CHAR_DATA *create_mobile( MOB_INDEX_DATA *pMobIndex )
{
	...
	mob->steel = number_range(wealth/200,wealth/50);
	if((wealth - (mob->steel * 100))>1000)
	   gwealth = number_range(100, 1000);
	else
	   gwealth = 0;
	mob->gold = gwealth;
    } 
...
void do_areas( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf(buf, "(Levels)  %-29s (Levels)  %-29s\n\r", "Area name", "Area name");
    send_to_char(buf, ch);
    ...
    for(pArea = area_first; pArea != NULL; pArea = pArea->next)
    {
    	...
    	if(pArea->high_range == 0)
    	    sprintf(buf, "(  ALL  ) %-29s ", pArea->name);
    		       /* (111-111) */
    	else
    	    sprintf(buf, "(%3d-%-3d) %-29s ", pArea->low_range, pArea->high_range, pArea->name);
    	send_to_char(buf, ch);
...
void do_dump( CHAR_DATA *ch, char *argument )
{
	...
	top_room, top_room * (sizeof(*room)));
     ...
     /* exits */
    fprintf(fp,"{W[{ME{mxits{W:{B %4d{W]{x (%8d bytes)\n",
...
char *center( char *str, int len )
{
    ...
    half	= ( (len - colorstrlen(str)) / 2 );
    sechalf	=   (len - (half + colorstrlen(str)));

    
    sprintf( strbuf, "%*c%s%*c",
       	half, ' ',
    	str,
    	sechalf, ' ' );
