...
char *	const	dir_name	[]		=
{
    "{cNorth{x", "{cEast{x", "{cSouth{x", "{cWest{x", "{cUp{x", "{cDown{x"
};
...
bool make_blood(CHAR_DATA *ch, int chance, bool show)
{
    if(number_range(1,chance) == 1)
    {
        OBJ_DATA *pObj;
        if((pObj = get_obj_list(ch, "blood1", ch->in_room->contents)) == NULL
        && get_obj_list(ch, "blood2", ch->in_room->contents) == NULL)
        {
     	    pObj = create_object(get_obj_index(OBJ_VNUM_SBLOOD), 0);
    	    obj_to_room(pObj, ch->in_room);
    	    pObj->timer = 5;
    	}
    	else if(get_obj_list(ch, "blood2", ch->in_room->contents) == NULL)
       	{
    	    extract_obj(pObj);
    	    pObj = create_object(get_obj_index(OBJ_VNUM_LBLOOD), 0);
    	    obj_to_room(pObj, ch->in_room);
    	    pObj->timer = 5;
    	}
    	if(show)
    	{
    	    act("$n drips some blood.", ch, NULL, NULL, TO_ROOM);
    	    send_to_char("You drip some blood on the ground.\n\r", ch);
    	}
    	return TRUE;
    }
    return FALSE;
...
void move_char( CHAR_DATA *ch, int door, bool follow )
{
	...
	/* Make sure a mounted player has a pet */
	if(!IS_NPC(ch) && (ch->pet == NULL || ch->pet->in_room->vnum != ch->in_room->vnum))
	{
	...
	else
	{
	    if( ch->pet->move < move )
	    {
	    	send_to_char( "Your pet is too tired.\n\r", ch);
	    	return;
	    }
    ...
    &&   (ch == (ch->master ? ch->master->pet : ch) && (ch->master ? ch->master->position : POS_STANDING) != POS_RIDING)
    &&	 ch->position != POS_RIDING
    &&   (!IS_NPC(ch) && !IS_SET(ch->pcdata->immflag, IMMF_ROOMWIZ)))
	act( "$n leaves $T.", ch, NULL, dir_name[door], TO_ROOM );
    
    if(ch->position == POS_RIDING)
    	act( "$n leaves $T, riding on $t.", ch, ch->pet->short_descr, dir_name[door], TO_ROOM );

    /* Blood code */
    if(ch->hit < ch->max_hit / 4)
    {
    	make_blood(ch, 3, TRUE);
    }
    ...
    if ( !IS_AFFECTED(ch, AFF_SNEAK)
    &&   ch->invis_level < LEVEL_HERO
    &&   ((ch->master ? ch->master->position : POS_STANDING) != POS_RIDING && ch == (ch->master ? ch->master->pet : ch))
    &&	 ch->position != POS_RIDING
    &&   (!IS_NPC(ch) && !IS_SET(ch->pcdata->immflag, IMMF_ROOMWIZ)))
	act( "$n has arrived.", ch, NULL, NULL, TO_ROOM );

    if ( ch->position == POS_RIDING )
    	act( "$n has arrived, riding on $t.", ch, ch->pet->short_descr, NULL, TO_ROOM );
	...
	if (IS_SET(ch->in_room->room_flags, ROOM_NOMAGIC))
	{
	   ...
	   spell_cancellation(skill_lookup("cancellation"), MAX_LEVEL + 50, ch, ch, TARGET_CHAR);
	   spell_cancellation(skill_lookup("cancellation"), MAX_LEVEL + 50, ch, ch, TARGET_CHAR);
	   spell_cancellation(skill_lookup("cancellation"), 500 , ch, ch, TARGET_CHAR);
	}
...
void do_mount( CHAR_DATA *ch, char *argument )
{
    ...
    {
    	send_to_char("You can't ride that pet.\n\r", ch);
    	return;
    }
    
    if(ch->position == POS_FIGHTING)
    {
    	send_to_char("You're too busy to mount.\n\r", ch);
    	return;
    }
    
    if(ch->position == POS_RIDING)
    {
    	send_to_char("You're already riding a pet.\n\r", ch);
    	return;
    }
    
    SET_BIT(ch->act, PLR_MOUNTED);
    ch->position = POS_RIDING;
    act("$n gets on $N.", ch, NULL, ch->pet, TO_ROOM);
    act("You get on $N.", ch, NULL, ch->pet, TO_CHAR);
}

void do_dismount( CHAR_DATA *ch, char *argument )
{
    if(ch->pet == NULL || !IS_SET(ch->act, PLR_MOUNTED))
    {
    	send_to_char("You try to dismount the air.\n\r", ch);
    	return;
    }
    
    if(ch->position == POS_FIGHTING)
    {
    	send_to_char("You're too busy to dismount right now.\n\r", ch);
    	return;
    }
    
    REMOVE_BIT(ch->act, PLR_MOUNTED);
    ch->position = POS_STANDING;
    act("$n gets off $N.", ch, NULL, ch->pet, TO_ROOM);
    act("You get off $N.", ch, NULL, ch->pet, TO_CHAR);
}
...
void do_sleep( CHAR_DATA *ch, char *argument )
{
    ...
    {
    case POS_RIDING:
    	send_to_char( "You can't ride and sleep at the same time.\n\r", ch);
    	break;
...
void do_recall( CHAR_DATA *ch, char *argument )
{
    ...
    {
    	send_to_char("You'll have to walk back.\n\r",ch);
    	return;
    }
  
    act( "$n prays for transportation!", ch, 0, 0, TO_ROOM );

    if ( ( location = get_room_index( (ch->recall != 0 ? ch->recall : (clan_table[ch->clan].recall != 0 ? clan_table[ch->clan].recall : god_table[ch->god].recall )) ) ) == NULL )
    {
  ...
  else
  {
    ...
    || (IS_NPC(victim) 
       &&	 (IS_SET(victim->act,ACT_TRAIN)
          ...
          ||	 IS_SET(victim->act,ACT_IS_CHANGER)
          ||	 IS_SET(victim->imm_flags,IMM_SUMMON)
          ||	 victim->pIndexData->pShop )) /* shop/healers */
       ||   victim->in_room == NULL
       ||   IS_SET(victim->in_room->room_flags, ROOM_SAFE)
       ||   IS_SET(victim->in_room->room_flags, ROOM_PRIVATE)
       ||   IS_SET(victim->in_room->room_flags, ROOM_SOLITARY)
...
void do_school( CHAR_DATA *ch, char *argument )
{
    ...
    if(ch->level > 5)
    {
    	send_to_char("You're a little too old for school.\n\r", ch);
    	return;
    }
    
    if((pRoom = get_room_index(ROOM_VNUM_SCHOOL)) == NULL)
    {
    	send_to_char("There's no such thing as school.\n\r", ch);
    	return;
    }
    
    act("$n went back to school.", ch, NULL, NULL, TO_ROOM);
    char_from_room(ch);
    char_to_room(ch, pRoom);
    act("$n walks into the school.", ch, NULL, NULL, TO_ROOM);
    pet_follow(ch, ch->pet);
    do_function(ch, &do_look, "auto");
}
