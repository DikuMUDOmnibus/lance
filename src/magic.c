...
void spell_imprint( int sn, int level, CHAR_DATA *ch, void *vo )
{
   ...
   /* scribe/brew costs 4 times the normal mana required to cast the spell */
	...
	break;
    case 2:
        if ( number_percent() > 70 || !IS_IMMORTAL(ch) )
        { 
          sprintf(buf, "The magic enchantment has failed --- the %s vanishes.\n\r", obj->name );
	  send_to_char( buf, ch );
	  extract_obj( obj );
...
void spell_resurrect( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    ...
    mob->level                  = ch->level + 9;
    mob->max_hit                = ch->level * 8 + number_range(
                                        mob->level * mob->level/1.625,
                                        mob->level * mob->level/1.375);

    mob->hit                    = mob->max_hit;
    mob->max_mana               = 100 + dice(mob->level,(mob->level + 10));
...
void do_cast( CHAR_DATA *ch, char *argument )
{
    ...
    {
    	send_to_char("There's something wrong with your voice.\n\r", ch);
    	return;
    }
/*
...
/* Advanced Mind created by Scott MacLean, Paladine */
void spell_advanced_mind(int sn,int level,CHAR_DATA *ch,void *vo,int target)
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    AFFECT_DATA af;
    ...
    {
        if (victim == ch)
          send_to_char("You're mind is already at its peak!\n\r",ch);
        else
          act("$N's mind is already at its peak!.",ch,NULL,victim,TO_CHAR);
        return;
    }
    ...
    af.modifier  = 1 + (level >= 9) + (level >= 18) + (level >= 27);
    af.bitvector = 0;
    affect_to_char( victim, &af );
    send_to_char( "Your mind power increases!\n\r", victim );
    act("$n's mindpower increases!.",victim,NULL,NULL,TO_ROOM);
    return;
}


void spell_aquastrike(int sn,int level,CHAR_DATA *ch,void *vo,int target)
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    int dam;

    dam = dice(8 + level / 2, 8);
...
void spell_blindness( int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
    ...
    {
    	act("It didn't seem to work on $N.",ch, NULL,victim,TO_CHAR);
	return;
    }
...
void spell_cancellation( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    if (check_dispel(level,victim,skill_lookup("advanced mind")))
	found = TRUE;
    ...
    if (check_dispel(level,victim,skill_lookup("enhanced mind")))
	found = TRUE;
    ...
    if (check_dispel(level,victim,skill_lookup("flameshield")))
    {
    	act("$n's fiery aura fades away.",victim,NULL,NULL,TO_ROOM);
    	found = TRUE;
    }
    ...
    if (check_dispel(level,victim,skill_lookup("lightningshield")))
    {
        act("The crackeling aura around $n's body vanishes.",
            victim,NULL,NULL,TO_ROOM);
        found = TRUE;
    }
    if (check_dispel(level,victim,skill_lookup("flaming eyes")))
    {
        act("The red in $n eyes, fade, and the fire dies.",
            victim,NULL,NULL,TO_ROOM);
        found = TRUE;
    }
    ...
    {
    	act("$n regains $s voice.",victim,NULL,NULL,TO_ROOM);
    	found = TRUE;
    }
...
void spell_cure_critical( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    ...
    if ( ch != victim )
	send_to_char( "They look a little better.\n\r", ch );
    return;
}
...
void spell_dispel_evil( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    ...
    if ( IS_GOOD(victim) )
    {
	act( "Something protects $N.", ch, NULL, victim, TO_ROOM );
	return;
    }
...
void spell_enchant_armor( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    ...
    if (result <= (90 - level/5) || !IS_IMMORTAL(ch))  /* success! */
    {
...
//        ch->alignment = UMAX(-1000, ch->alignment - 50);
    ...
    if ( saves_spell( level-20, victim,DAM_NEGATIVE) )
    {
	send_to_char("You feel a momentary chill.\n\r",victim);  	
	send_to_char("It looks like the spell didn't work.\n\r",ch);
	return;
    }
	...
	send_to_char("It seems their life is slipping away.\n\r",ch);
	dam		 = ch->hit + 1;
    }
    else
    {
...
void spell_consume( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
	int num1, num2;
    int dam;
	...
	num1 = ( 2 * level ) - 14;
	num2 = ( 2 * level ) + 16;
	dam		 = number_range(num1, num2);
	ch->hit		+= dam;
	if (ch->hit > ch->max_hit)
		ch->hit = ch->max_hit;
    send_to_char("You scream in agony as your life slips away\n\r",victim);
    send_to_char("You steal some life from them...\n\r",ch);
    damage( ch, victim, dam, sn, DAM_NEGATIVE ,TRUE);
...
void spell_enhanced_mind(int sn,int level,CHAR_DATA *ch,void *vo,int target)
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    AFFECT_DATA af;
    ...
    {
        if (victim == ch)
          send_to_char("You can only be so wise!\n\r",ch);
        else
          act( "$N is wise, leave him alone.",ch,NULL,victim,TO_CHAR);
        return;
    }
    ...
    af.type      = sn;
    af.level     = level;
    af.duration  = level - 1 / 2 + 5;
    af.location  = APPLY_WIS;
    af.modifier  = 1 + (level >= 25) + (level >= 50);
    af.bitvector = 0;
    affect_to_char( victim, &af );
    send_to_char( "As enlightenment passes through you, you feel wise!\n\r", victim );
    act("$n's wisdom increases!.",victim,NULL,NULL,TO_ROOM);
    return;
}
...
void spell_flamepillar( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    CHAR_DATA *vch, *vch_next;
    int dam;

    act("$n is encompassed by a pillar of flame!",ch,NULL,victim,TO_NOTVICT);
    act("$n gestures, summoning a pillar of flame which engulfs you!",ch,NULL,victim,TO_VICT);
    act("You gesture, sending a pillar of flame that engulfs the vacinity!",ch,NULL,NULL,TO_CHAR);
...
void spell_ethreal_blade( int sn, int level,CHAR_DATA *ch,void *vo,int target )
{
    OBJ_DATA *disc, *floating, *sham;
    sham  = get_eq_char(ch, WEAR_LEFT);
    if(sham == NULL || sham->item_type != ITEM_GEM)
	{
    ...
    if(sham != NULL && sham->item_type == ITEM_GEM)
	{
	   act("You draw upon the power of the spell $p.",ch,sham,NULL,TO_CHAR);
	   act("It flares and vanishes.",ch,sham,NULL,TO_CHAR);
	   extract_obj(sham);
	}

    floating = get_eq_char(ch,WEAR_RIGHT);

    disc = create_object(get_obj_index(OBJ_VNUM_EBLADE), 0);
    disc->value[1]	= ch->level / 5; /* 10 pounds per level capacity */
    disc->value[2]	= ch->level / 5; /* 5 pounds per level max per item */
    disc->timer		= ch->level / 5; 

    act("$n has created a dagger of pure magic.",ch,NULL,NULL,TO_ROOM);
    send_to_char("You create a dagger of pure magic.\n\r",ch);
    obj_to_char(disc,ch);
    wear_obj(ch,disc,TRUE);
    return;
}
...
void spell_gate( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    ||	 IS_SET(ch->in_room->room_flags, ROOM_NO_TRANS)
...
void spell_hallucinate( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    bool gate_pet;
    AFFECT_DATA af;
	...
	switch (level)
	{
	case 0:		location = get_room_index( ROOM_HAL_1 );
	case 1:		location = get_room_index( ROOM_HAL_1 );
	case 2:		location = get_room_index( ROOM_HAL_1 );
	case 3:		location = get_room_index( ROOM_HAL_1 );
	case 4:		location = get_room_index( ROOM_HAL_1 );
	default:	location = get_room_index( ROOM_HAL_1 );
	}
	...
	ch->trip_room = ch->in_room;

    
    act("$n stares into oblivion, then disappears.",ch,NULL,NULL,TO_ROOM);
    send_to_char("AH!!! What the hell???\n\r",ch);
    char_from_room(ch);
    char_to_room(ch,location);

    act("$n has joined you in the world of hallucination...",ch,NULL,NULL,TO_ROOM);
    do_function(ch, &do_look, "auto");

    if (gate_pet)
    {
	act("$n stops dead in his tracks and disappears.",ch->pet,NULL,NULL,TO_ROOM);
	send_to_char("AHH!!! What the Hell???.\n\r",ch->pet);
	char_from_room(ch->pet);
	char_to_room(ch->pet,location);
	act("$n has joined you in the world of hallucination...",ch->pet,NULL,NULL,TO_ROOM);
	do_function(ch->pet, &do_look, "auto");
    }
}
...
void spell_identify( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    if (IS_OBJ_STAT(obj,ITEM_NOIDEN))
    {
	sprintf( buf, "{CThis information is forbiden to the mortals of Krynn.\n\r{x");
	send_to_char( buf, ch );
        if(ch->level<=101)
	   return;
    }
...
void spell_iron_shards( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    int dam;

    dam = dice(9 + level / 2, 8 );
    if ( saves_spell( level, victim, DAM_IRON ) )
        dam /= 2;
    damage( ch, victim, dam, sn, DAM_IRON ,TRUE);
    return;
}
...
void spell_mana_charge(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
    CHAR_DATA *pch = (CHAR_DATA*) vo;
    
    pch->mana += pch->in_room->mana_rate + ch->level;
    act("A warm glow passes through $n.", pch, NULL, NULL, TO_ROOM);
    send_to_char("Your blood tingles as you feel restored.\n\r", pch);
}
...
void spell_silver_shards( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    int dam;

    dam = dice(9 + level / 2, 9 );
    if ( saves_spell( level, victim, DAM_SILVER ) )
        dam /= 2;
...
void spell_summon( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    ||	 IS_SET(victim->in_room->room_flags, ROOM_NO_TRANS)
	...
	{
	send_to_char( "They are not able to be summoned right now.\n\r",ch );
	return;
	}
...
/* True Sight Spell added by Scott MacLean, Paladine */
void spell_true_sight( int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
    ...
    {
	if (victim == ch)
	  send_to_char("Trying to go blind? You already can see.\n\r",ch);
	else
	  act("$N can already see truly!",ch,NULL,victim,TO_CHAR);
	return;
    }
    ...
    affect_to_char( victim, &af );
    send_to_char( "Your eyes glow brilliantly!\n\r", victim );
    act("$n's eyes glow brightly!.",victim,NULL,NULL,TO_ROOM);
    return;
...
void spell_word_of_recall( int sn, int level, CHAR_DATA *ch,void *vo,int target)
{
    ...
    if ((location = get_room_index( (ch->recall ? ch->recall : (clan_table[ch->clan].recall != 0 ? clan_table[ch->clan].recall : god_table[ch->god].recall )) )) == NULL)
    {
    ...
    {
    	send_to_char("You are too excited to recall.\n\r", ch );
    	return;
    }
...
void spell_find_familiar (int sn, int level,CHAR_DATA *ch, void *vo, int target)
{
   ...
   || (!IS_NPC(ch) 
   && ch->level < skill_table[gsn_familiar].skill_level[ch->class]))
   {
   ...
   if(ch->position == POS_FIGHTING)
{
   ...
   if ( ( pMobIndex = get_mob_index(MOB_VNUM_FAMILIAR) ) == NULL )
   {
    ...
    mount->hit = mount->max_hit = number_range(
    ch->max_hit * 2,
    ch->max_hit * 3) + number_range(mount->level * 2,mount->level * 3);
    for(i = 0; i < 4; i++)
    mount->armor[i] = number_fuzzy(ch->armor[i] - 10);
    mount->hitroll = number_fuzzy(ch->level / 3);
    mount->damroll = number_fuzzy(ch->level / 3);
...
/* terrain */
	...
	case(SECT_CITY): /* rat */
	case(SECT_FIELD):
	    mount->description = str_dup("You see a large furry rat.  Long whiskers hang down from it's nose.\n\r"
	                  "You can feel the dirt and disease crawling off this beast.\n\r");
	    mount->short_descr = str_dup("A large rat");
	...
	case(SECT_HILLS):
	    ...
	    mount->short_descr = str_dup("A large falcon");
	    mount->long_descr = str_dup("A large falcon screams here.\n\r");
	    mount->name = str_dup("familiar large falcon");
	    ...
	    mount->short_descr = str_dup("A large mountain lion");
...
/* player seen stuff here */
    ...
    act( "You begin to chant and call to $N!.",ch,NULL,mount,TO_CHAR);
    act( "$n begins to chant and calls to $N!", ch, NULL, mount, TO_ROOM );
...
//        free_string(mount->short_descr);
//        mount->short_descr = str_dup(capitalize(target_name));
//    }
...
void spell_lightningshield(int sn,int level, CHAR_DATA *ch, void *vo,int target)
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    AFFECT_DATA af;
    ...
    {
    	if(victim == ch)
    	    send_to_char("You already have a lightning shield.\n\r", ch);
    	else
    	    send_to_char("Target already has a lightning shield.\n\r", ch);
	return;    	
    }
    ...
    af.location  = APPLY_NONE;
    af.modifier  = 0;
    af.bitvector = AFF_LIGHTNING;
    affect_to_char( victim, &af );
    act("A crackeling aura surrounds $n.",victim,NULL,NULL,TO_ROOM);
    act("You are surrounded by a crackeling aura.",victim,NULL,NULL,TO_CHAR);
    return;
}
...
void spell_flameshield(int sn,int level, CHAR_DATA *ch, void *vo,int target)
{
    ...
    {
    	if(victim == ch)
    	    send_to_char("You already have a flameshield.\n\r", ch);
    	else
    	    send_to_char("Target already has a flameshield.\n\r", ch);
	return;    	
    }
    ...
    affect_to_char( victim, &af );
    act("A fiery aura surrounds $n.",victim,NULL,NULL,TO_ROOM);
    act("You are surrounded by a fiery aura.",victim,NULL,NULL,TO_CHAR);
    return;
}
...
void spell_feast(int sn,int level, CHAR_DATA *ch, void *vo,int target)
{
    ...
    for(rch = ch->in_room->people; rch != NULL; rch = rch->next_in_room)
    {
    	if(IS_NPC(rch))
    	    continue;
    	if(rch->pcdata->condition[COND_THIRST] < 50)
    	    rch->pcdata->condition[COND_THIRST] = 50;
    	if(rch->pcdata->condition[COND_HUNGER] < 50)
    	    rch->pcdata->condition[COND_HUNGER] = 50;
    	send_to_char("You feel satiated!\n\r", rch);
    }
    send_to_char("Ok.\n\r", ch);
    return;
...
void spell_silence(int sn,int level, CHAR_DATA *ch, void *vo,int target)
{
    ...
    if(IS_AFFECTED2(pch, AFF_SILENCE))
    {
    	act("$N is already silenced.",ch,NULL,pch,TO_CHAR);
    	return;
    }

    if (saves_spell(level,pch,DAM_ENERGY))
    {
	if (ch == pch)
	  send_to_char("You feel momentarily mute, but it passes.\n\r",pch);
	else
	  act("$N seems to still have their voice",ch,NULL,pch,TO_CHAR);
	return;
    }
    ...
    act("$n appears to be silenced.",pch,NULL,NULL,TO_ROOM);
    send_to_char("You feel your voice slip away.\n\r", pch);
}

void spell_voice(int sn,int level, CHAR_DATA *ch, void *vo,int target)
{
    CHAR_DATA *pch = (CHAR_DATA *) vo;
    
    if(!IS_AFFECTED2(pch, AFF_SILENCE))
    {
    	act("$N doesn't seem to be silenced.",ch,NULL,pch,TO_CHAR);
    	return;
    }
    
    if(check_dispel(level,pch,skill_lookup("silence")))
    	act("$n regains $s voice.",pch,NULL,NULL,TO_ROOM);
    else
    {
    	send_to_char("It didn't seem to work.\n\r", ch);
    	send_to_char("Your throat feels tingly for a moment.\n\r", pch);
    }
}
...
void spell_scrye(int sn,int level,CHAR_DATA *ch,void *vo,int target)
{
    ...
    if((wch = get_char_world(ch, target_name)) == NULL)
    {
    	send_to_char("They aren't here.\n\r", ch);
    	return;
    }
    
    if(ch->level < wch->level)
    {
    	send_to_char("You failed.\n\r", ch);
    	return;
    }
    if(IS_SET(wch->in_room->room_flags, ROOM_NO_SCRYE))
       {
	send_to_char("Something protects the room they reside in.\n\r",ch);
	return;
       }

    chance = ch->pcdata->learned[sn] - wch->saving_throw/2;
    if(chance > number_percent())
    {
    	ROOM_INDEX_DATA *pLRoom;
    	
    	pLRoom = ch->in_room;
    	ch->in_room = wch->in_room;
    	do_function(ch, &do_look, "auto");
    	if(chance/3 > number_percent())
    	    act("You feel $n's presence in the room.",ch,NULL,NULL,TO_ROOM);
    	ch->in_room = pLRoom;
    }
    else
    {
...
void spell_mini_meteor(int sn,int level,CHAR_DATA *ch,void *vo,int target)
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    int i = ch->level < 10 ? 1 : ch->level / 10 + 1;
    for(; i > 0; i--)
    {
...
void spell_flaming_eyes( int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    AFFECT_DATA af;

    if ( IS_AFFECTED2(victim, AFF_FLAME_EYE) )
    {
	if (victim == ch)
	  send_to_char("Your eyes are already immune to fire.\n\r",ch);
	else
	  act("$N already has impervious eyes!",ch,NULL,victim,TO_CHAR);
	return;
    }
    ...
    af.location  = APPLY_SAVES;
    af.modifier  = -5;
    af.bitvector = AFF_FLAME_EYE;
    affect_to_char( victim, &af );
    send_to_char( "Your eyes glow red, and feel protected!\n\r", victim );
    act("$n's eyes glow red!",victim,NULL,NULL,TO_ROOM);
    return;

}

void spell_acid_darts( int sn, int level, CHAR_DATA *ch,void *vo,int target)
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    int dam,i;
...
void spell_chaos_flare( int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
	...
	send_to_char( "Scales form over your skin!\n\r", victim);

	if(ch != victim)
	    act("$N's skin is suddenly coverd in reptillian scales.",ch,NULL,victim,TO_CHAR);
	return;
    }
    if(rnum <= 15)
    {
	...
	affect_to_char( victim, &af );
	send_to_char("Sharp spikes shoot out of your skin!\n\r", victim);
	if(ch != victim)
	    act("$N's skin is now coverd in spikes!\n\r",ch,NULL,victim,TO_CHAR);
	return;
    }
    if(rnum <= 25)
    {
	...
	send_to_char( "Your legs grow an extra foot long!\n\r", ch);
	if(ch!=victim)
	    act("Wierd, $N's legs grow an extra foot long!",ch,NULL,victim,TO_CHAR);
	return;
    }
    if(rnum <= 45)
    {
	...
	send_to_char( "You become much tougher!\n\r", ch);
	if(ch!=victim)
	    act("$N suddently seems tougher!",ch,NULL,victim,TO_CHAR);
	return;
    }
    if(rnum <= 50)
    {
	...
	send_to_char( "Your muscles double in size!\n\r", ch);
	if(ch!=victim)
	    act("$N's muscles seem to double in size.",ch,NULL,victim,TO_CHAR);
	return;
    }
    if(rnum <= 65)
    {
	...
	if(ch!=victim)
	    act("$N suddenly looks very stupid!",ch,NULL,victim,TO_CHAR);
	return;
    }
    if(rnum <= 85)
    {
	...
	send_to_char( "You suddenly grow two sizes larger!\n\r", ch);
	if(ch!=victim)
	    act("$N gets bigger...",ch,NULL,victim,TO_CHAR);
	return;
    }
    if(rnum <= 95)
    {
	...
	if(ch!=victim)
	    act("$N looks mighty paranoid all of the sudden.",ch,NULL,victim,TO_CHAR);
	return;
    }
    if(rnum <= 100)
    {
	...
	send_to_char( "Ack! If feels as if your bones have disapeared\n\r", ch);
	if(ch!=victim)
	    act("Eww It looks as if $N's bones have left his body.",ch,NULL,victim,TO_CHAR);
	return;
    }
	return;
...
void spell_holy_wrath( int sn, int level, CHAR_DATA *ch,void *vo,int target)
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    int dam,i;
