...
void spell_portal( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    ...
    stone = get_eq_char(ch,WEAR_LEFT);
    if (!IS_IMMORTAL(ch) 
...
void spell_nexus( int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
    ...
    ||	 IS_SET(from_room->room_flags, ROOM_NO_TRANS)
    ||   victim->level >= level + 3
    ||   (!IS_NPC(victim) && victim->level >= LEVEL_HERO)  /* NOT trust */
    ||   (IS_NPC(victim) && IS_SET(victim->imm_flags,IMM_SUMMON))
    ||   (IS_NPC(victim) && saves_spell( level, victim,DAM_NONE) ) 
...
void spell_holy_weapon( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    ...
    stone = get_eq_char(ch,WEAR_LEFT);
    if (!IS_IMMORTAL(ch) 
    &&  (stone == NULL || stone->item_type != ITEM_GEM))
    {
	send_to_char("You lack the proper component for this spell.\n\r",ch);
	return;
    }

    if (stone != NULL && stone->item_type == ITEM_GEM)
    {
     	act("You draw upon the holy power of the $p.",ch,stone,NULL,TO_CHAR);
     	act("It flares brightly and vanishes!",ch,stone,NULL,TO_CHAR);
     	extract_obj(stone);
    }

    portal = create_object(get_obj_index(OBJ_VNUM_HOLYBLADE),0);
    portal->value[1] = (ch->level/5);
    portal->value[2] = (ch->level/5);
    portal->timer    = 2 + level / 5; 
	act("You have created a weapon of holy power.",ch,stone,NULL,TO_CHAR);
    obj_to_char(portal,ch);
    wear_obj(ch,portal,TRUE);
}
