...
void	apply_mod	args( ( CHAR_DATA *ch, sh_int stat, int mod ) );

/* Makes the pet follow its owner in cases where it might not */
void pet_follow(CHAR_DATA *ch, CHAR_DATA *pet)
{
    if(pet == NULL)
    	return;
    	
    act("$n follows $s master.", pet, NULL, NULL, TO_ROOM);
    char_from_room(pet);
...
/* for immunity, vulnerabiltiy, and resistant
   ...
   three other cases -- wood, gold, and iron -- are checked in fight.c */
...
int get_weapon_sn(CHAR_DATA *ch, int hand)
{
    OBJ_DATA *wield;
    int sn;
	...
	for ( af = obj->pIndexData->affected; af != NULL; af = af->next )
        {
            ...
            switch(af->location)
            {
		case APPLY_STR:		apply_mod( ch, STAT_STR, mod );	break;
		case APPLY_DEX:		apply_mod( ch, STAT_DEX, mod ); break;
		case APPLY_INT:		apply_mod( ch, STAT_INT, mod ); break;
		case APPLY_WIS:		apply_mod( ch, STAT_WIS, mod ); break;
		case APPLY_CON:		apply_mod( ch, STAT_CON, mod ); break;
...
{
    int age;
    if( (age = pc_race_table[ch->race].age) == 0)
    	age = 17;
    	
    return age + ( ch->played + (int) (current_time - ch->logon) ) / 72000;
}
...
int apply_ac( OBJ_DATA *obj, int iWear, int type )
{
    ...
    case WEAR_ANKLE:	return     obj->value[type];
    case WEAR_EAR:	return     obj->value[type];
    case WEAR_BACK:     return     obj->value[type];
    case WEAR_TATTOO_L: return     obj->value[type];
    case WEAR_TATTOO_R: return     obj->value[type];
...
void equip_char( CHAR_DATA *ch, OBJ_DATA *obj, int iWear )
{
    ...
    {
    	if(IS_NPC(ch))
    	    bug( "Equip_char: room %d.", ch->in_room->vnum);
...
void unequip_char( CHAR_DATA *ch, OBJ_DATA *obj )
{
	...
	if ( paf->location == APPLY_SPELL_AFFECT )
	{
	    ...
	    bug ( "Norm-Apply: %d", 0 );
	    sprintf(buf, "%s, %d", ch->name, obj->pIndexData->vnum);
	    bug(buf, 0);
	    append_file(ch, OBJ_BUG_FILE, buf);
	    for ( lpaf = ch->affected; lpaf != NULL; lpaf = lpaf_next )
	    {
		lpaf_next = lpaf->next;
		if ((lpaf->type == paf->type) &&
...
void extract_char( CHAR_DATA *ch, bool fPull )
{
    ...
    */
    ...
    if ( !fPull )
    {        
    	if( clan_table[ch->clan].hall == ROOM_VNUM_TEMPLE )
    	{
    	    int vnum;
    	    ...
    	    else /* god_table[ch->god].pc_neutral */
    ...
    if(!IS_NPC(ch) && IS_SET(ch->act, PLR_QUESTOR))
...
CHAR_DATA *get_char_room( CHAR_DATA *ch, char *argument )
{
	...
	if ( !can_see( ch, rch ) || !is_name( arg, rch->name ) )
	    continue;
	if(!IS_NPC(rch) && IS_SET(rch->pcdata->immflag, IMMF_ROOMWIZ))
	    continue;
	if ( ++count == number )
	    return rch;
    }
...
OBJ_DATA *get_obj_list( CHAR_DATA *ch, char *argument, OBJ_DATA *list )
{
    ...
    for ( obj = list; obj != NULL; obj = obj->next_content )
    {
	if ( can_see_obj( ch, obj ) && is_name( arg, obj->name ) && obj->safe != 1 )
	{
	    if ( ++count == number )
		return obj;
	}
...
void deduct_cost(CHAR_DATA *ch, int cost)
{
    ...
    if (ch->steel < 0)
    {
	bug("deduct costs: steel %d < 0",ch->steel);
	ch->steel = 0;
    }
    if (ch->gold < 0)
    {
...
/* RT changed so that WIZ_INVIS has levels */
    ...
    ||   (IS_AFFECTED(ch, AFF_DETECT_INVIS) && ch->level < victim->level)))
	return FALSE;
...
char *affect2_bit_name( int vector )
{
    ...
    buf[0] = '\0';
    if ( vector & AFF_ENHANCED_MIND	) strcat( buf, " enhanced_mind"	);
    if ( vector & AFF_ADVANCED_MIND	) strcat( buf, " advanced_mind"	);
    if ( vector & AFF_FLAMESHIELD	) strcat( buf, " flameshield"	);
    if ( vector & AFF_SILENCE		) strcat( buf, " silence"	);
	if ( vector & AFF_HALLUCINATING ) strcat( buf, " hallucination"	);
    return ( buf[0] != '\0' ) ? buf+1 : "none";
}
...
char *act_bit_name( int act_flags )
{
	...
	if (act_flags & PLR_MOUNTED	) strcat(buf, " mounted");
    }
    return ( buf[0] != '\0' ) ? buf+1 : "none";
}
...
char *act2_bit_name( int act2_flags )
{
    ...
    if( act2_flags & ACT_BANKER		) strcat( buf, " banker" );
    if( act2_flags & ACT_GAMBLER	) strcat( buf, " gambler" );
    if( act2_flags & ACT_NO_ATTACK	) strcat( buf, " no_attack" );
    if( act2_flags & ACT_NO_COUNTER	) strcat( buf, " no_counter" );

    return ( buf[0] != '\0' ) ? buf+1 : "none";
}

char *immf_bit_name(int immf_flags)
{
    static char buf[512];
    
    buf[0] = '\0';
    
    if(immf_flags & IMMF_ROOMWIZ)	strcat( buf, " roomwizi" );
    return (buf[0] != '\0') ? buf+1 : "none";
}
...
char *class_bit_name( int class_flags )
{
    ...
    buf[0] = '\0';
    if( class_flags & ITEM_MAGE_ONLY	) strcat( buf, " mage" );
    if( class_flags & ITEM_CLERIC_ONLY	) strcat( buf, " cleric" );
    if( class_flags & ITEM_THIEF_ONLY	) strcat( buf, " thief" );
    if( class_flags & ITEM_WARRIOR_ONLY	) strcat( buf, " warrior" );
    if( class_flags & ITEM_PALADIN_ONLY	) strcat( buf, " paladin" );
    if( class_flags & ITEM_RANGER_ONLY	) strcat( buf, " ranger" );
    if( class_flags & ITEM_TEMPLAR_ONLY	) strcat( buf, " templar" );
    if( class_flags & ITEM_BMAGE_ONLY	) strcat( buf, " bmage" );
...
char *clanrank_name( CHAR_DATA * ch )
{
    char rankname[MAX_STRING_LENGTH];
    int clan = ch->clan;
    int subclan = ch->subclan;
    int ranknum = ch->clanlevel;

    rankname[0] = '\0';

    if( ch->clanrank != NULL )
    {
    	strcpy( rankname, ch->clanrank );
    	return ( rankname[0] != '\0' ) ? rankname : "";
    }


    if(clan == clan_lookup("solamnic"))
    {
    	if( ch->clanlevel == 11 )
    	{
    		strcpy( rankname, "Grandmaster" );
    	}

    	switch( subclan )
    	{
    	case 1:
    		strcpy( rankname, sol_crown_table[ranknum].name );
    		break;
    	case 2:
    		strcpy( rankname, sol_sword_table[ranknum].name );
    		break;
    	case 3:
    		strcpy( rankname, sol_rose_table[ranknum].name );
    		break;
    	}
    }
    else if(clan == clan_lookup("kot"))
    {
    	if( ch->clanlevel >= 10 )
    	{
    		strcpy( rankname, "Lord of the Night" );
    	}

    	switch( subclan )
    	{
    	case 5:
    		strcpy( rankname, kot_lily_table[ranknum].name );
    		break;
    	case 4:
    		strcpy( rankname, kot_skull_table[ranknum].name );
    		break;
    	case 7:
    		strcpy( rankname, kot_thorn_table[ranknum].name );
    		break;
        case 6:
        	strcpy( rankname, kot_storm_table[ranknum].name );
        	break;

 	}
    }
    else if(clan == clan_lookup("holyorder"))
    {
    	if( ch->clanlevel >= 10 )
    	{
    		strcpy( rankname, "High Master Healer" );
    	}

    	switch( subclan )
    	{
    	case 12:
    		strcpy( rankname, ho_light_table[ranknum].name );
    		break;
    	case 13:
    		strcpy( rankname, ho_balance_table[ranknum].name );
    		break;
    	case 14:
    		strcpy( rankname, ho_dark_table[ranknum].name );
    		break;

 	}
    }

    else if(clan == clan_lookup("conclave"))
    	strcpy( rankname, conclave_table[ranknum].name );
    else if(clan == clan_lookup("wildrunner"))
    	strcpy( rankname, wildrunner_table[ranknum].name );
    else if(clan == clan_lookup("mercenary"))
	strcpy( rankname, mercenary_table[ranknum].name );
    else if(clan == clan_lookup("thehorde"))
    	strcpy( rankname, mithril_table[ranknum].name );
    else if(clan == clan_lookup("finder"))
    	strcpy( rankname, handler_table[ranknum].name );
    else if(clan == clan_lookup("rfos"))
    	strcpy( rankname, queshu_table[ranknum].name );
    else if(clan == clan_lookup("undead"))
    	strcpy( rankname, undead_table[ranknum].name );
    else if(clan == clan_lookup("teyr"))
    	strcpy( rankname, teyr_table[ranknum].name );
    else if(clan == clan_lookup("thieves"))
        strcpy( rankname, thieves_table[ranknum].name );
...
void apply_mod( CHAR_DATA *ch, sh_int stat, int mod )
{
    sh_int prime;
...
/*    if( (ch->perm_stat[stat] + (ch->mod_stat[stat] += mod)) > pc_race_table[ch->race].max_stats[stat] + prime )
	ch->mod_stat[stat] = pc_race_table[ch->race].max_stats[stat] + prime - pc_race_table[ch->race].stats[stat];
*/
    ch->mod_stat[stat] += mod;
    return;
}
