...
void save_char_obj( CHAR_DATA *ch )
{
    ...
    if(!IS_VALID(ch))
    {
        bug("save_char_obj: can't save invalid char.", 0);
        return;
    }
    ...
    if( !ch->pcdata->confirm_delete )
    	...
    	sprintf( strsave, "%s%s", PLAYER_BAK, capitalize( ch->name ) );
...
fwrite_char( CHAR_DATA *ch, FILE *fp )
{
    ...
    fprintf( fp, "LogO %ld\n",	current_time		);
    fprintf( fp, "Vers %d\n",   5			);
    fprintf( fp, "Host %s~\n", ch->pcdata->orig_host[0] != '\0' ?
    	ch->pcdata->orig_host : ch->desc->host		);
    fprintf( fp, "FName %s~\n",	
        ch->pcdata->fngr_name[0] != '\0'
        ? ch->pcdata->fngr_name
        : "none");
    fprintf( fp, "FEmail %s~\n",	
    	ch->pcdata->fngr_email[0] != '\0'
    	? ch->pcdata->fngr_email
    	: "none");
    fprintf( fp, "FIcq %s~\n",
    	ch->pcdata->fngr_icq[0] != '\0'
    	? ch->pcdata->fngr_icq
    	: "none");
    fprintf( fp, "FAim %s~\n",
    	ch->pcdata->fngr_aim[0] != '\0'
    	? ch->pcdata->fngr_aim
    	: "none");
    fprintf( fp, "FMSN %s~\n",
    	ch->pcdata->fngr_msn[0] != '\0'
    	? ch->pcdata->fngr_msn
    	: "none");
    fprintf( fp, "FLastHost %s~\n",
    ch->pcdata->fngr_lasthost[0] != '\0'
    ? ch->pcdata->fngr_lasthost
    : "none");	
    fprintf( fp, "LastLogon %ld\n", ch->logon		);
  ...
  if(ch->pcdata->spouse != NULL)
	...
	fprintf( fp, "Subclan %s~\n",subclan_table[ch->subclan].name);
    ...
    fprintf( fp, "LStory %ld\n", ch->pcdata->last_story	);
    fprintf( fp, "LWar %ld\n", ch->pcdata->last_war     );
    fprintf( fp, "LStaff %ld\n", ch->pcdata->last_staff );
    fprintf( fp, "Scro %d\n", 	ch->lines		);
    fprintf( fp, "Room %d\n",
	...
	ch->pcdata->pkill, ch->pcdata->pdeath, ch->pcdata->mkill, 
ch->pcdata->mdeath );
    ...
    if(ch->pcdata->immflag != 0)
    	fprintf(fp, "Immf %s\n",    print_flags(ch->pcdata->immflag));
    fprintf( fp, "Comm %s\n",       print_flags(ch->comm));
    if (ch->wiznet)
	...
	if (ch->pcdata->transin[0] != '\0')
		fprintf( fp, "TransIn %s~\n",	ch->pcdata->transin);
	if (ch->pcdata->transout[0] != '\0')
		fprintf( fp, "TransOut %s~\n",	ch->pcdata->transout);
	fprintf( fp, "Titl %s~\n",	ch->pcdata->title	);
    	fprintf( fp, "Pnts %d\n",   	ch->pcdata->points      );
	fprintf( fp, "TSex %d\n",	ch->pcdata->true_sex	);
...
*/
	...
	if(ch)//<--- This was all that was added. Just to make sure ch isn't null.
...
bool load_char_obj( DESCRIPTOR_DATA *d, char *name )
{
    ...
    ch->pcdata->lastlogon		= 0;
    ch->pcdata->orig_host		= str_dup( "" );
    ch->pcdata->fngr_name		= str_dup( "" );
    ch->pcdata->fngr_email		= str_dup( "" );
    ch->pcdata->fngr_icq		= str_dup( "" );
    ch->pcdata->fngr_aim		= str_dup( "" );
    ch->pcdata->fngr_msn		= str_dup( "" );
    ch->pcdata->fngr_lasthost		= str_dup( "" );
    ch->pcdata->learned[gsn_right_hand] = 1;
    ch->pcdata->learned[gsn_left_hand]	= 1;
    ch->pcdata->remorts			= 0;
    ch->pcdata->bounty			= 0;
	...
	switch (ch->level)
	{
	    case(40) : ch->level = (MAX_LEVEL - 0);	break;  /* imp -> imp */
        ...
        switch (ch->trust)
        {
            case(40) : ch->trust = (MAX_LEVEL - 0);  break;	/* imp -> imp */
            ...
            case(36) : ch->trust = (MAX_LEVEL - 9);  break;	/* hero -> hero */
        }
    }
...
fread_char( CHAR_DATA *ch, FILE *fp )
{
	...
	case 'B':
	    ...
	    KEY( "BSteel",	ch->pcdata->banksteel,	fread_number( fp ) );
	    KEY( "BGold",	ch->pcdata->bankgold,	fread_number( fp ) );
	...
	case 'C':
	    ...
	    KEY( "Clanlevel",	ch->clanlevel,		fread_number( fp ) );
	    KEY( "Clanrank",	ch->clanrank,		fread_string( fp ) );
	...
	case 'E':
	    ...
	    break;
	case 'F':
	    KEY( "FAim",	ch->pcdata->fngr_aim,	fread_string( fp ) );
	    KEY( "FName",	ch->pcdata->fngr_name,	fread_string( fp ) );
	    KEY( "FEmail",	ch->pcdata->fngr_email,	fread_string( fp ) );
	    KEY( "FIcq",	ch->pcdata->fngr_icq,	fread_string( fp ) );
	    KEY( "FLastHost",	ch->pcdata->fngr_lasthost, fread_string( fp ) );
	    KEY( "FMsn",	ch->pcdata->fngr_msn,	fread_string( fp ) );
	    break;
	case 'G':
            KEY( "Gold",        ch->gold,               fread_number( fp ) );
	    KEY( "God",		ch->god,		fread_number( fp ) );
	    KEY( "Grant",	ch->grant,		fread_string( fp ) );
	...
	case 'H':
	    ...
	    KEY( "Host",	ch->pcdata->orig_host,	fread_string( fp ) );
	...
	case 'I':
	    ...
	    KEY( "Immf",	ch->pcdata->immflag,	fread_flag( fp )   );
	...
	case 'L':
	    ...
	    KEY( "LastLogon",	ch->pcdata->lastlogon,	fread_number( fp ) );
	    ...
	    KEY( "LStory",	ch->pcdata->last_story,	fread_number( fp ) );
            KEY( "LWar",        ch->pcdata->last_war,   fread_number( fp ) );
            KEY( "LStaff",      ch->pcdata->last_staff, fread_number( fp ) );
	    break;
	...
	case 'P':
	    ...
	    KEY( "Pretitle",	ch->pretitle,		fread_string( fp ) );
            KEYS( "Prompt",      ch->prompt,             fread_string( fp ) );
 	    KEYS( "Prom",	ch->prompt,		fread_string( fp ) );
	    if ( !str_cmp( word, "PkPdMkMd" ) )
	    {
		ch->pcdata->pkill	= fread_number( fp );
		ch->pcdata->pdeath	= fread_number( fp );
	...
	case 'R':
	    ...
	    KEY( "Remorts",	ch->pcdata->remorts,	fread_number( fp ) );
	...
	case 'S':
	    KEY( "SafeBox",	ch->pcdata->safeboxes,	fread_number( fp ) );
	    ...
	    KEY( "Subclan",	ch->subclan,		subclan_lookup(fread_string(fp)));
	...
	case 'W':
	    KEYS ("WNc", ch->pcdata->wiziname, fread_string (fp));
	    KEYS ("WNs", ch->pcdata->storedwiziname, fread_string (fp));
          KEY ("WNt", ch->pcdata->wizinametoggle, fread_number (fp));
	    KEY( "Weight", ch->pcdata->weight, fread_flag( fp ) );
	    KEY( "Wimpy",	ch->wimpy,		fread_number( fp ) );
	    KEY( "Wimp",	ch->wimpy,		fread_number( fp ) );
	    KEY( "Wizn",	ch->wiznet,		fread_flag( fp ) );
	    KEY( "Whotext",	ch->whotext,		fread_string( fp ) );
	    break;
	}
...
*/
	...
	case 'S':
	    KEY( "Safe",	obj->safe,fread_number( fp ) );
