    ...
    for( iHash = 0; iHash < MAX_KEY_HASH; iHash++ )
    {
        for( pRoomIndex = room_index_hash[iHash]; pRoomIndex; pRoomIndex = pRoomIndex->next )
        {
            if ( pRoomIndex->area == pArea )
            {
                fprintf( fp, "#%d\n",		pRoomIndex->vnum );
                fprintf( fp, "%s~\n",		pRoomIndex->name );
                fprintf( fp, "%s~\n",		fix_string( pRoomIndex->description1 ) );
            ...
            if ( pRoomIndex->area == pArea )
            {
                for( door = 0; door < MAX_DIR; door++ )
                {
                          ...
                          || IS_SET( pExit->rs_flags, EX_HIDDEN ) ) )
                    {
                    	...
                    	{
                    		num = 4;
                    		sprintf( buf, "closed, locked and hidden" );
                    	}
                    	else if( !IS_SET( pExit->rs_flags, EX_LOCKED )
                    	&&	  IS_SET( pExit->rs_flags, EX_HIDDEN ) )
                    	{
                    		num = 3;
                    		sprintf( buf, "closed and hidden" );
                    	}
                    	else if( !IS_SET( pExit->rs_flags, EX_HIDDEN )
                    	&&	  IS_SET( pExit->rs_flags, EX_LOCKED ) )
                    	{
                    		num = 2;
                    		sprintf( buf, "closed and locked" );
                    	}
                    	else
                    	{
                    		num = 1;
...
/*    {
	...
	switch (ch->desc->editor)
	{
		...
		break;
	    case ED_HELP:
		send_to_char( "Recording area : ", ch );
	    	save_other_helps( ch );
	    	return;
	    default:
		pArea = ch->in_room->area;
