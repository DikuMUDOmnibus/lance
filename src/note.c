...
void do_unread(CHAR_DATA *ch)
{
    ...
    {
    	found = TRUE;
    	sprintf(buf,"There %s %d new stor%s waiting.\n\r",
    	    count > 1 ? "are" : "is",
    	    count,
    	    count > 1 ? "ies" : "y");
    	send_to_char(buf, ch);
    }
    if ((count = count_spool(ch,war_list)) > 0)
    {
        found = TRUE;
        sprintf(buf,"There %s %d new war action%s waiting.\n\r",
        count > 1 ? "are" : "is",
        count,
        count > 1 ? "s" : "");
       send_to_char(buf, ch);
    }
    if (ch->level >= IMMORTAL && (count = count_spool(ch,staff_list)) > 0)
    {
       found = TRUE;
       sprintf(buf,"There %s %d new staff note%s waiting.\n\r",
       count > 1 ? "are" : "is",
       count,
       count > 1 ? "s" : "");
      send_to_char(buf, ch);
    ...
    if (IS_TRUSTED(ch,BUILDER) && (count = count_spool(ch,penalty_list)) > 0)
    {
	found = TRUE;
...
{
     parse_note(ch,argument,NOTE_WAR);
}

void do_staff(CHAR_DATA *ch,char *argument)
{
     parse_note(ch,argument,NOTE_STAFF);
}
...
void load_notes(void)
{
    ...
    load_thread(WAR_FILE,&war_list,NOTE_WAR,0);
    load_thread(STAFF_FILE,&staff_list,NOTE_STAFF,0);
}
...
bool hide_note (CHAR_DATA *ch, NOTE_DATA *pnote)
{
	...
	case NOTE_STORY:
	    ...
	    break;
        case NOTE_WAR:
            last_read = ch->pcdata->last_war;
            break;
        case NOTE_STAFF:
            last_read = ch->pcdata->last_staff;
            break;
    }
...
void update_read(CHAR_DATA *ch, NOTE_DATA *pnote)
{
    ...
    switch (pnote->type)
    {
        ...
        case NOTE_STORY:
            ...
            break;
        case NOTE_WAR:
            ch->pcdata->last_war = UMAX(ch->pcdata->last_war,stamp);
            break;
        case NOTE_STAFF:
            ch->pcdata->last_staff = UMAX(ch->pcdata->last_staff,stamp);
            break;
    }
}
...
void parse_note( CHAR_DATA *ch, char *argument, int type )
{
	...
	default:
        ...
        case NOTE_WAR:
            list = &war_list;
            list_name = "war actions";
            break;
        case NOTE_STAFF:
            list = &staff_list;
            list_name = "immortal board";
            break;
    }
	...
	if (!vnum)
	{
		...
		case NOTE_CHANGES:
		    ...
		    break;
		case NOTE_STORY:
		    send_to_char("There are no new stories.\n\r",ch);
		    break;
	        case NOTE_WAR:
                    send_to_char("There are no new war actions.\n\r",ch);
                    break;
                case NOTE_STAFF:
                    send_to_char("There are no new staff messages.\n\r",ch);
                    break;
            }
	}
	return;
	...
	switch(type)
	{
	    ...
	    case NOTE_STORY:
            ...
            case NOTE_WAR:
                ...
                break;
            case NOTE_STAFF:
                ch->pcdata->last_staff = current_time;
                break;
	}
	return;
    }
    ...
    if ((type == NOTE_NEWS && !IS_TRUSTED(ch,BUILDER))
    ||  (type == NOTE_CHANGES && !IS_TRUSTED(ch,CODER)))
    {
	...
	if(ch->level < 2)
	{
	    send_to_char("You must be level 2 before you can write notes.\n\r", ch);
	    return;
	}
	
	if(IS_SET(ch->comm, COMM_NONOTE))
	{
	    send_to_char("You are not allowed to post notes.\n\r", ch);
	    return;
	}
		...
		case NOTE_STORY:
			...
			break;
                case NOTE_WAR:
                        ...
                        break;
                case NOTE_STAFF:
                        strcpy( buf1, "staff message" );
                        break;
	}
	sprintf( buf, "{g%s has posted a new %s.{x\n\r", ch->name, buf1);		
	for(d = descriptor_list; d != NULL; d = d->next)
	{
	     if(d->connected == CON_PLAYING
	     && IS_IMMORTAL(d->character)
	     && IS_SET(d->character->wiznet, WIZ_POSTS)
