...
void do_gain(CHAR_DATA *ch, char *argument)
{
	...
	sprintf(buf, "{C%-18s {y%-5s {W%-18s {r%-5s {m%-18s {g%-5s{x\n\r",
	...
	col = 0;
        ...
        sprintf(buf, "{b%-18s {w%-5s {Y%-18s {R%-5s {M%-18s {G%-5s{x\n\r",
    ...
    if(!str_prefix(arg, "study"))
    {
    	if(ch->train < 1)
    	{
    	    act("$N tells you 'You aren't ready for that.'",
    	    	ch,NULL,trainer,TO_CHAR);
    	    return;
    	}
    	
    	act("$N teaches you how to practice.",
    		ch,NULL,trainer,TO_CHAR);
    	ch->practice += 10;
    	ch->train -= 1;
    	return;
...
void do_spells(CHAR_DATA *ch, char *argument)
{
	...
	&&  ch->pcdata->learned[sn] > 0)
	    ...
	    if (ch->level < level)
	    	sprintf(buf,"{y%-18s{x {Wn/a{x      ", skill_table[sn].name);
	    else
	    {
		mana = UMAX(skill_table[sn].min_mana,
		    100/(2 + ch->level - level));
	        sprintf(buf,"{y%-18s  {g%3d mana{x  ",skill_table[sn].name,mana);
	    }
 
	    if (spell_list[level][0] == '\0')
          	sprintf(spell_list[level],"\n\r{mLevel {D%2d:{x {b%s{x",level,buf);
	    else /* append */
	    {
...
void do_skills(CHAR_DATA *ch, char *argument)
{
	...
	&&  ch->pcdata->learned[sn] > 0)
	    ...
	    level = skill_table[sn].skill_level[ch->class];
	    if (ch->level < level)
	    	sprintf(buf,"{y%-18s {Wn/a{x      ", skill_table[sn].name);
	    else
	    	sprintf(buf,"{y%-18s {g%3d%%{x      ",skill_table[sn].name,
		    ch->pcdata->learned[sn]);
 
	    if (skill_list[level][0] == '\0')
          	sprintf(skill_list[level],"\n\r{mLevel {D%2d: {b%s{x",level,buf);
	    else /* append */
	    {
